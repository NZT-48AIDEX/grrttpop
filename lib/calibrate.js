...
/**
 * "never fires" is only evidence with enough independent observations.
 ...
 * die. Below this many *moments* it is reported as information, not
 * as a complaint.
 *
 * Moments, not samples: the reef and the solana ecosystem taken from
 * one snapshot are two rows of one observation of one market, and
 * counting them as two would reach this threshold twice as fast while
 * knowing nothing more. A sample carries `at` to say which moment it
 * belongs to.
 */
...
/** how much real time the samples cover, which is not the same as how many */
function spanOf(samples) {
  const times = samples.map((s) => (s.at ? new Date(s.at).getTime() : null)).filter(Boolean).sort();
  if (times.length < 2) return null;
  return { from: new Date(times[0]).toISOString(), to: new Date(times.at(-1)).toISOString(),
           hours: +((times.at(-1) - times[0]) / 3_600_000).toFixed(1) };
}
...
export function calibrate(samples = [], { deadLabelSamples = DEAD_LABEL_SAMPLES } = {}) {
  ...
  /* a sample with no `at` is its own moment — the caller did not say
     otherwise, and assuming rows share one would undercount */
  const moments = new Set(usable.map((s, i) => s.at ?? `#${i}`));
  ...
  for (const l of labels) {
    ...
    } else if (l.count === 0 && moments.size >= deadLabelSamples) {
      flags.push({ kind: "never-fires", label: l.label, share: 0,
        why: `never fired across ${moments.size} moments — either the threshold is unreachable or the case does not occur` });
  ...
  if (coins > 0 && breadth.divergent === 0 && moments.size >= deadLabelSamples) {
  ...
  return {
    ...
    /* the number that actually decides whether any of this is evidence */
    ...
    enoughForDeadLabels: moments.size >= deadLabelSamples,
    note: moments.size < deadLabelSamples
      ? `${usable.length} sample(s) over ${moments.size} moment(s) — labels that did not fire ` +
        `here are not dead, just unmet: nothing falls on a rising day. ` +
        `needs ${deadLabelSamples}+ moments spread over real days before retuning anything.`
      : `${usable.length} samples over ${moments.size} moments`,
  };
}
