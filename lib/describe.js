...
/* the sparkline is the last 7 days scaled to this coin's own range, so
   it says nothing about size and everything about shape. the label only
   appears when the week and the last day point opposite ways — that's
   the case where reading the 24h number alone gets you the wrong idea. */
const coinLine = (c, arrow) =>
  `  ${arrow} ${sym(c.symbol)} ${String(pct(c.change24h)).padStart(8)}  ` +
  `${String(fmtPrice(c.price)).padEnd(12)}` +
  (c.trend ? ` ${c.trend.spark}` : "") +
  (c.trend?.divergent ? `  ← 7d: ${c.trend.shape}` : "") +
  (c.trending ? "  🔥 trending" : "");

/* the market as one line: an equal-weighted index of everyone's week,
   plus breadth, because one index number hides whether a small move is
   everybody drifting or one coin carrying the whole thing. */
const weekLines = (w, subject) => {
  if (!w) return [];
  const b = w.breadth;
  return [
    "",
    `the week — 7 days, hourly, ${w.coinsCounted} of them`,
    `  ${subject.padEnd(26)}${w.spark}  ${pct(w.weekPct)} — ${w.shape}`,
    /* "sideways" rather than "flat": most of these moved plenty, they
       just ended up where they started. */
    `  ${b.up} up · ${b.down} down · ${b.flat} sideways` +
      (b.divergent ? ` · ${b.divergent} spent the week going one way and today the other` : ""),
  ];
};

export function reefToText(d, { live = false } = {}) {
  const L = [];
  L.push("the reef — the live crypto market, as an ocean");
  L.push("");
  /* the same stamp the json carries, so the two can't disagree about
     whether what follows is real */
  L.push(provenanceLine(d.data));
  if (d.data?.note) L.push(`⚠️  ${d.data.note}`);
  L.push("");
  ...
  /* three states, not two: a snapshot has no socket at all, and "not
     connected" would read as a broken heartbeat rather than an absent one */
  L.push(`heartbeat: ${live === null ? "n/a — a snapshot has no socket; the live trade pulses only exist in a browser"
    : live ? "live — binance trades are pulsing through the creatures" : "not connected"}`);
  L.push("");
  ...
  L.push(...weekLines(d.week, "the reef as one creature"));
  L.push("");
...
export function trenchToText(d) {
  ...
  L.push(...weekLines(e.week, "the ecosystem as one"));
