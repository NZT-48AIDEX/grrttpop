...
const manifest = ({ reef, trench, at, every }) => ({
  ...
  freshness: "every file carries a `data` block with `source`, `asOf` and `synthetic`. " +
             "read it. a snapshot is minutes old by design, and nothing here is ever " +
             "published from the demo reef or from recorded fixtures.",
  live: SITE,
 ...
 */
export function buildSnapshot({ reef = null, trench = null, at = Date.now(), every = "30 minutes" } = {}) {
