...
/* ---------------- fetching ---------------- */
/* the error carries the status and a bite of the body: an incident that
   records only "coingecko 429" tells you the what and never the why, and
   the whole point of capturing a failure is that it is hard to reproduce
   on purpose later. */
...
export function describeReef(coins, { trending = new Set(), mood = null, global: g = null, sort = "mcap", data = null } = {}) {
  ...
  return {
    ...
    global: g,
    /* where these numbers came from — including when the honest answer
       is "they are invented, the api was down" */
