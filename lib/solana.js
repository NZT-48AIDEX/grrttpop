...
export async function peekWallet(addr, { rpc, limit = 24, fetch: f = fetch, onEvent = () => {} } = {}) {
  ...
  return {
    ...
    partial: accountsFailed,
    gated,
    reason: !accountsFailed ? null
      : gated ? "free public RPCs block token-account reads — add your own endpoint (free key from helius/quicknode) to see the whole wallet"
      : "rpc unreachable for token accounts — SOL balance only",
  };
}

/** what the trench would look like, for something that cannot see it */
export function describeTrench({ vitals = {}, eco = [], wallet = null } = {}) {
  const brief = (c) => ({
