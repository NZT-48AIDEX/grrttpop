...
export function install({ name, renderer: r } = {}) {
  ...
  if (renderer?.debug) {
    renderer.debug.onShaderError = (gl, program, vs, fs) => {
      shaderFails++;
      // gl info logs are null-terminated c strings — strip it, or every
      // compile log an agent reads ends in \u0000
      const info = (s) => s?.replace(/\0/g, "").trim() || null;
      track("shader", info(gl.getProgramInfoLog(program)) || "shader link failed", {
        vertex: info(gl.getShaderInfoLog(vs)),
        fragment: info(gl.getShaderInfoLog(fs)),
      });
    };
  }
