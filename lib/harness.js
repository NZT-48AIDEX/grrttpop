...
import { routeFor, RPC_FIXTURE } from "./fixture-routes.js";
...
if (HARNESS.fixtures) {
  globalThis.fetch = async (input, init) => {
    ...
    if (body && typeof body === "string" && body.includes("jsonrpc")) {
      ...
      const all = await fixture(RPC_FIXTURE);
    ...
    if (name) return json(await fixture(name));
...
/* ---------------- reduced motion ---------------- */
/**
 * The whole site is movement — a noise-displaced blob, drifting dust, a
 * companion chasing the cursor, a camera that swims. For someone with
 * vestibular sensitivity that is not a delight, it's a reason to close the
 * tab. `prefers-reduced-motion` is them saying so.
 *
 * The answer here is stillness, not absence: the creature stays, the colours
 * stay, the data stays live. It just stops lurching. `?motion=full` overrides
 * for anyone who set the system preference for other reasons and wants this
 * anyway.
 */
